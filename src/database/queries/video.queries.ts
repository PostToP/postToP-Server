...
export class VideoQueries {
  static async fetch(videoID: string) {
    ...
    return db.selectFrom("posttop.video").selectAll().where("yt_id", "=", videoID).executeTakeFirst();
  }
  ...
  static async insert(
    ...
    const exists = await trx.selectFrom("posttop.video").select("id").where("yt_id", "=", videoID).executeTakeFirst();
  ...
  private static async getQueryForAll(params: QueryForAllParams) {
    ...
    let query = db
      ...
      .innerJoin(
        db.selectFrom("posttop.channel").select(["id", "name"]).as("channel"),
        "posttop.video.channel_id",
        "channel.id",
      );
    ...
    if (params.filters?.verified !== undefined) {
      ...
      if (params.filters.verified) {
        query = query.where(
          "posttop.is_music_video.submitted_by_id",
          "in",
          db
            .selectFrom("posttop.user_role")
            .innerJoin("posttop.role", "posttop.user_role.role_id", "posttop.role.id")
        ...
        query = query.where(eb =>
          eb.or([
            eb(
              "posttop.is_music_video.video_id",
              "not in",
              db
                .selectFrom("posttop.is_music_video")
    ...
    if (params.filters?.hasNER !== undefined) {
      if (params.filters.hasNER) {
        query = query.where("posttop.video.id", "in", db.selectFrom("posttop.ner_result").select("video_id"));
      } else {
        query = query.where("posttop.video.id", "not in", db.selectFrom("posttop.ner_result").select("video_id"));
      }
    }
