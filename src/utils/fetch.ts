...
export async function fetchJsonWithRetry<T>(
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    logger.warn(
      `fetchJson failed (attempt ${attempt + 1}/${retries}) [${method}] ${url}: ${result.error.message}. Retrying in ${delay}ms...`,
    );
    await new Promise(resolve => setTimeout(resolve, delay));
  }
  const method = options.method ?? "GET";
  logger.error(
    `fetchJson failed after ${retries} attempts [${method}] ${url} with payload ${JSON.stringify(options.params ?? options.body ?? {})}`,
  );
